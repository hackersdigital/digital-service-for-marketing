# app/views/portal.py
from flask import Blueprint, render_template, request, g, Response, make_response, current_app
...
def _build_query(client, form, status, q, date_from, date_to):
    query = {}
    if client:
        query["client_slug"] = client
    if form:
        query["form_slug"] = form
    if status:
    ...
    if date_from or date_to:
        ...
        if date_to:
            try:
                rng["$lte"] = _dt_end_of_day(datetime.strptime(date_to, "%Y-%m-%d"))
            except ValueError:
                pass
        if rng:
...
def portal_home():
    ...
    # Latest delivery per submission (so we can show outbound request body)
    delivery_map = {}
    if submissions:
    ...
    # If a correct ?token= is present, set a short-lived portal_token cookie
    tok = (request.args.get("token", "") or "").strip()
    expected = (current_app.config.get("PORTAL_TOKEN") or "").strip()
    if tok and tok == expected:
        # 1 hour expiry; Lax is enough for cross-page form posts on same site
        resp.set_cookie(
            "portal_token",
            tok,
            httponly=True,
...
def portal_export_csv():
    # Cookie/Bearer/query token are all accepted by require_portal_token
    require_portal_token()
